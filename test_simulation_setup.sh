...
# ROS2 Nomad Simulation Test Setup Script

echo "Setting up ROS2 Nomad simulation environment..."
...
mkdir -p test_worlds
echo "Created test_worlds directory"

# Create simple test world
cat > test_worlds/simple_room.world << 'EOF'
<?xml version="1.0" ?>
<sdf version="1.4">
...
echo "Created simple test world: test_worlds/simple_room.world"

# Create launch file for simulation
cat > launch/simulation_launch.py << 'EOF'
#!/usr/bin/env python3
...
def generate_launch_description():
    ...
    world_arg = DeclareLaunchArgument(
        'world',
        default_value=PathJoinSubstitution([pkg_share, 'test_worlds', 'simple_room.world']),
    ...
    gazebo_server = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            PathJoinSubstitution([gazebo_ros_pkg_share, 'launch', 'gzserver.launch.py'])
        ]),
    ...
    gazebo_client = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            PathJoinSubstitution([gazebo_ros_pkg_share, 'launch', 'gzclient.launch.py'])
        ])
    )
    
    # Spawn robot (TurtleBot3)
    spawn_robot = Node(
        package='gazebo_ros',
        executable='spawn_entity.py',
        arguments=[
            '-entity', 'turtlebot3_burger',
            '-file', PathJoinSubstitution([FindPackageShare('turtlebot3_gazebo'), 'models', 'turtlebot3_burger', 'model.sdf']),
            '-x', '0.0',
            '-y', '0.0',
            '-z', '0.0'
        ],
    ...
    robot_state_publisher = Node(
        ...
        arguments=[PathJoinSubstitution([FindPackageShare('turtlebot3_bringup'), 'urdf', 'turtlebot3_burger.urdf.xacro'])],
        output='screen'
    )
    ...
    # Nomad nodes (from main launch file)
    nomad_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            PathJoinSubstitution([pkg_share, 'launch', 'nomad_launch.py'])
        ]),
        launch_arguments={
...
echo "Created simulation launch file: launch/simulation_launch.py"

# Create RViz configuration for simulation
mkdir -p config
cat > config/simulation.rviz << 'EOF'
Panels:
...
echo "Created RViz configuration: config/simulation.rviz"

echo "Simulation setup completed successfully!"
echo ""
echo "To run simulation:"
echo "1. Build the package: ./build_ros2.sh"
echo "2. Source the workspace: source install/setup.bash"
echo "3. Run simulation: ros2 launch nomad_ros2 simulation_launch.py"
echo ""
echo "To run with different modes:"
echo "  Navigation: ros2 launch nomad_ros2 simulation_launch.py mode:=navigate"
echo "  Exploration: ros2 launch nomad_ros2 simulation_launch.py mode:=explore" 