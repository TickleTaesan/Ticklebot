...
def main(args=None):
    ...
    node.declare_parameter('joint_names', [ "joint_l1", "joint_l2", "joint_l3", "joint_l4", "joint_l5", "joint_l6", "joint_l7l" ])
    ...
    def cb(msg: astra_controller_interfaces.msg.JointCommand):
        ...
        send_msg = sensor_msgs.msg.JointState()
        send_msg.header.stamp = node.get_clock().now().to_msg()
        send_msg.name = joint_names[6:7]
        send_msg.position = [ msg.position_cmd[0] ]
        joint_state_publisher.publish(send_msg)
