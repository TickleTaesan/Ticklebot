...
let pedalKeyMapping = {
  'ArrowLeft': 'angular-neg',    // 왼쪽 화살표
  'ArrowRight': 'angular-pos',   // 오른쪽 화살표
  'ArrowDown': 'linear-neg',     // 아래 화살표
  'ArrowUp': 'linear-pos'        // 위 화살표
};

let isPedalConnected = false;
let pedalInterval = null;

async function connectPedal() {
  // HID 페달은 캘리브레이션 없이 바로 사용 가능
  
  document.getElementById('connect-pedal').classList.add("hidden");
  document.getElementById('calibrate-pedal').classList.add("hidden");
  toastr.success("HID Pedal connected. Use Arrow Keys: ←→ for angular, ↑↓ for linear");
  
  document.getElementById('pedal-status').innerHTML = 'HID Pedal connected (Arrow Keys)';

  const fromServerCb = async function (evt) {
    console.dir(evt.detail);
    // HID 페달은 서버로부터 명령을 받지 않음
  }
  ...
  pedalInterval = setInterval(() => {
    if (!isPedalConnected) return;
    
    // 페달 상태를 아날로그 값으로 변환 (0.0 ~ 1.0)
    const pedalRealValues = [
      pedalStates['angular-pos'] ? 1.0 : 0.0,  // angular-pos
      pedalStates['angular-neg'] ? 1.0 : 0.0,  // angular-neg
      pedalStates['linear-neg'] ? 1.0 : 0.0,   // linear-neg
      pedalStates['linear-pos'] ? 1.0 : 0.0    // linear-pos
    ];
    
    // UI 업데이트
    for (const i in pedalNames) {
      document.getElementById('pedal-' + pedalNames[i]).value = pedalRealValues[i] * 100;
    }
...
function disconnectPedal() {
  ...
  // 모든 페달 상태 초기화
  for (let key in pedalStates) {
    pedalStates[key] = false;
  }
  
  pedalCommTarget.removeEventListener('fromServer', () => {});
  ...
  toastr.success("Pedal disconnected.");
}

async function calibratePedal() {
  // HID 페달은 캘리브레이션이 필요 없음 (디지털 입력)
  toastr.info("HID Pedal doesn't need calibration. Just connect and use!", undefined, { timeOut: 3000 });
  
  // 키 매핑 안내
  toastr.info(`
    <strong>Key Mapping:</strong><br>
    ← (Left Arrow): Angular Negative<br>
    → (Right Arrow): Angular Positive<br> 
    ↓ (Down Arrow): Linear Negative<br>
    ↑ (Up Arrow): Linear Positive<br>
    <small>You can change these in the browser console if needed.</small>
...
window.addEventListener('load', function () {
  ...
  document.addEventListener("keydown", (event) => {
    // 페달 키 처리 (반복 방지)
    if (isPedalConnected && pedalKeyMapping[event.key] && !event.repeat) {
      const pedalName = pedalKeyMapping[event.key];
      pedalStates[pedalName] = true;
      console.log(`Pedal ${pedalName} pressed`);
  ...
  document.addEventListener("keyup", (event) => {
    if (isPedalConnected && pedalKeyMapping[event.key]) {
      const pedalName = pedalKeyMapping[event.key];
      pedalStates[pedalName] = false;
      console.log(`Pedal ${pedalName} released`);
