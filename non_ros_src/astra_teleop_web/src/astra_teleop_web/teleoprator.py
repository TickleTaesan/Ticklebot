...
class Teleopoperator:
    def __init__(self):        
        ...
        # ESP32 WiFi 전송 (기존 UDP_Client 스타일)
        self.esp32_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.esp32_ip = None  # ESP32 IP는 동적으로 찾거나 설정
        self.esp32_port = 8888
        
    def send_to_esp32(self, pedal_data):
        """ESP32로 페달 데이터 전송 (기존 UDP_Client 스타일)"""
        data = {"pedal": pedal_data}
        ...
        try:
            ...
            if self.esp32_ip:
                # 특정 IP로 전송
                self.esp32_sock.sendto(message, (self.esp32_ip, self.esp32_port))
            else:
                # 브로드캐스트로 ESP32 찾기
                broadcast_ips = ['192.168.0.255', '192.168.1.255', '192.168.219.255']
                for ip in broadcast_ips:
                    try:
                        self.esp32_sock.sendto(message, (ip, self.esp32_port))
                    except:
                        pass
                        
        except Exception as e:
            logger.debug(f"ESP32 전송 실패: {e}")
            
    def set_esp32_ip(self, ip):
        """ESP32 IP 수동 설정"""
        self.esp32_ip = ip
        logger.info(f"ESP32 IP 설정: {ip}")
    ...
    def pedal_cb(self, pedal_real_values):
        # ESP32로 페달 데이터 전송
        self.send_to_esp32(pedal_real_values)
