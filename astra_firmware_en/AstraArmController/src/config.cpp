...
This file manages configuration storage and retrieval for the AstraArmController firmware. It provides functions to read, write, and print configuration parameters using the LittleFS filesystem, ensuring persistent and consistent settings for motion control and filtering algorithms.
*/
#include "config.h"
#include <Arduino.h>
#include <FS.h>
...
void print_config() {
  ...
  // Serial.print("7 joint_backlash_compensate_feedforward: "); Serial.println(config.joint_backlash_compensate_feedforward); // Not used anymore
  // Serial.print("8 non_joint_vel_max: "); Serial.println(config.non_joint_vel_max); // Not used anymore
  // Serial.print("9 non_joint_acc: "); Serial.println(config.non_joint_acc); // Not used anymore
}
