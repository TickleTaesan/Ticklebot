...
This is the main entry point for the AstraArmController firmware. It initializes configuration, display, dual motor control, and communication subsystems, then enters a loop to process incoming commands, update motor positions, and provide feedback. The code manages configuration storage, torque control, EIShaper filtering, and real-time communication, ensuring precise arm movement and system integration.
*/
...
#define FLOAT_TO_U32(x) (*(uint32_t *)&(x))
```
This macro reinterprets a `float` value as a 32-bit unsigned integer by:
1. Taking the address of the float (`&(x)`)
2. Casting it to a `uint32_t` pointer (`(uint32_t *)`)
3. Dereferencing it (`*`) to get the raw bits

### U32_TO_FLOAT(x)
```c
#define U32_TO_FLOAT(x) (*(float *)&(x))
```
This does the opposite - reinterprets a 32-bit unsigned integer as a `float` using the same technique.

### U32_TO_I32(x)
```c
#define U32_TO_I32(x) (*(int *)&(x))
```
This reinterprets a 32-bit unsigned integer as a signed integer.

### Important Notes:
- These macros perform type punning through pointers, which can violate strict aliasing rules
- They rely on:
  - `float` being 32-bits
  - Little/big endian consistency
  - Platform-specific floating-point representation (IEEE 754)
- In this code, they're used primarily for network communication and configuration storage where preserving the exact bit pattern is important
- A safer modern approach would be to use `std::memcpy` or `union` types for type punning

Example usage in the code:
```cpp
config.EIShaper_freq = U32_TO_FLOAT(cmd[1]); // Converts network byte order uint32 to float
cmd[1] = FLOAT_TO_U32(config.EIShaper_freq); // Converts float back to uint32 for network transfer
```
*/
